...
int map_conv(float *lon,                    // 경도(degree)
		float *lat,                    // 위도(degree)
		float *x,                      // X격자 (grid)
		float *y,                      // Y격자 (grid)
...
int lamcproj(float *lon, float *lat, /* Longitude, Latitude [degree]  */
				float *x, float *y, /* Coordinate in Map   [grid]    */
...
int GridToWgs84(int x, int y, float* lon, float* lat)
{
	...
	float x_f = (float)x;
	float y_f = (float)y;
	map_conv(lon, lat, &x_f, &y_f, 1, map);
...
int Wgs84ToGrid(float lon, float lat, int* x, int* y)
{
	...
	map.first = 0;
	float x_f, y_f;
	map_conv(&lon, &lat, &x_f, &y_f, 0, map);
	*x = (int)x_f;
	*y = (int)y_f;
	return 0;
}
